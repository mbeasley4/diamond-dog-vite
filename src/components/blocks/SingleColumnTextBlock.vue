<script setup lang="ts">
import { computed } from 'vue'

const props = defineProps<{
  data: {
    text: string,
  }
}>()

const sanitizedHtml = computed(() => {
  return props.data.text
    .replace(/\s*data-start="[\d]+"/g, '')
    .replace(/\s*data-end="[\d]+"/g, '')
    .replace(/\s*rel="noopener"/g, '') // Optional: clean other unwanted attrs
})
</script>
 
<template>
  <section class="relative overflow-hidden bg-brand-light single-column-text-brand-dark">
    <div class="relative max-w-7xl mx-auto px-6 py-20" v-html="sanitizedHtml"></div>
  </section>
</template>

<style scoped>
.single-column-text-brand-dark :deep(h1) {
  @apply text-4xl md:text-5xl font-bold mb-6;
}

.single-column-text-brand-dark :deep(h2) {
  @apply text-2xl md:text-3xl font-semibold mt-12 mb-4 border-b border-brand-dark/10 pb-2;
}

.single-column-text-brand-dark :deep(p) {
  @apply mb-4 leading-relaxed;
}

.single-column-text-brand-dark :deep(ul) {
  @apply ml-6 mb-6 space-y-2;
}

.single-column-text-brand-dark :deep(li) {
  @apply list-disc marker:text-brand-dark/60;
}

.single-column-text-brand-dark :deep(hr) {
  @apply my-8 border-t-2 border-brand-dark/20;
}

.single-column-text-brand-dark :deep(strong) {
  @apply font-semibold text-brand-dark;
}

.single-column-text-brand-dark :deep(em) {
  @apply italic text-brand-dark/80;
}

.single-column-text-brand-dark :deep(.decorated-link) {
  @apply text-blue-600 hover:text-blue-800 underline decoration-2 underline-offset-2 transition-colors;
}
</style>